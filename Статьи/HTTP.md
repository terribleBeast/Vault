...
Характеристики: 
1) Простой (для работы используется простой текст, понятный для человека)
2) Расширяемый (хэдеры могут расширяться бесконечно)
3) Сессия вместо состояния (сессия хранится в cookies) 
...
Функции: 
1. кэширование данных - для ускорения получения запросов (поэтому говорят: "почисти кэш", то есть браузер отправил ответ на предыдущий запрос)
2. фильтрация
3. аутентификация (протоколирование, логи)
...
#### GET
Используется для запроса содержимого с указанного ресурса. С помощью метода `GET` можно также начать какой-либо процесс. В этом случае в тело ответного сообщения следует включить информацию о ходе выполнения процесса.
...
4) [Наглядное видео](https://www.youtube.com/watch?v=sboscOZzOO0)